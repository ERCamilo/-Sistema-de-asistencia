...
export function FloatingCard() {
    ...
    return `<div class="overlay" onclick="window.closeFloatingCard()"></div><div class="floating-card" onclick="event.stopPropagation()"><div class="floating-card-header"><div class="floating-card-title">👤 ${emp.name}</div><button class="floating-card-close" onclick="window.closeFloatingCard()">✕</button></div><div class="stats-compact"><div class="stat-compact"><div class="stat-compact-label">Últimos 7 días</div><div class="stat-compact-value">${h7}h</div></div><div class="stat-compact"><div class="stat-compact-label">Este mes</div><div class="stat-compact-value">${hm}h</div></div></div><div class="calendar-compact"><div class="calendar-nav"><button class="calendar-nav-btn" onclick="window.changeFloatingMonth(-1)">◀</button><div class="calendar-month">${formatMonthYear(state.floatingCardMonth)}</div><button class="calendar-nav-btn" onclick="window.changeFloatingMonth(1)">▶</button></div><div class="calendar-header">${dayN.map(d => `<div class="calendar-header-day">${d}</div>`).join('')}</div><div class="calendar-grid">${days.map(({ date, currentMonth }) => {
    ...
    }).join('')}</div></div><div class="chart-compact"><div class="chart-compact-header"><div class="chart-compact-title">📈 ${state.chartPeriod === 'all' ? 'Historial por Meses' : 'Asistencia y Horas'}</div><div class="chart-filter"><button class="chart-filter-btn ${state.chartPeriod === 'week' ? 'active' : ''}" onclick="window.changeChartPeriod('week')">7D</button><button class="chart-filter-btn ${state.chartPeriod === 'month' ? 'active' : ''}" onclick="window.changeChartPeriod('month')">Mes</button><button class="chart-filter-btn ${state.chartPeriod === 'all' ? 'active' : ''}" onclick="window.changeChartPeriod('all')">Todo</button></div></div><div class="chart-bars">${chartData.map(d => {
    ...
    <div style="padding: 16px; border-top: 1px solid #334155;"><button onclick="window.openEmployeeProfile('${emp.id}')" style="width: 100%; padding: 12px; background: linear-gradient(135deg, #06b6d4, #10b981); border: none; border-radius: 8px; color: #000; font-weight: 700; cursor: pointer; font-size: 0.875rem; transition: all 0.2s;">👤 Ver Perfil Completo</button></div></div>`;
}
