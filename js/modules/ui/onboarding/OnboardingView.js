...
export function topbar(stepCounter, showSkip, progress = null) {
    const skip = showSkip ? `<button data-act="goLast" class="odv-skip-btn" style="height:32px;padding:0 12px;border-radius:8px;border:none;background:transparent;color:${C.faint};cursor:pointer;font-size:12.5px;font-weight:500;white-space:nowrap;">Omitir guía</button>` : '';
    ...
    return `<div class="odv-topbar" data-od-id="od-topbar" style="position:relative;display:flex;align-items:center;justify-content:space-between;padding:16px 24px;border-bottom:1px solid ${C.border};"><div class="odv-topbar-brand" style="display:flex;align-items:center;gap:11px;min-width:0;"><img src="icon-512.png" alt="" width="28" height="28" onerror="this.style.display='none'" style="width:28px;height:28px;border-radius:7px;display:block;flex:none;"><div style="line-height:1.15;min-width:0;"><div class="odv-topbar-title" style="font-size:13.5px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Control de Asistencia</div><div class="odv-topbar-subtitle" style="font-size:11px;color:${C.faint};">Contrutek</div></div></div><div class="odv-topbar-right" style="display:flex;align-items:center;gap:12px;flex:none;"><span style="font-family:'IBM Plex Mono',monospace;font-size:11.5px;color:${C.faint};">${esc(stepCounter)}</span>${skip}</div>${bar}</div>`;
}
export function footer(s, showDots, hint, nextLabel, last) {
    let dots = '';
    if (showDots) {
        dots = '<div class="odv-dots-container" style="display:flex;align-items:center;gap:6px;">' + STEPS.map((st, n) =>
            `<button data-no-min-target data-act="dot" data-n="${n + 1}" class="odv-dot" title="${esc(st.title)}" aria-label="Ir al paso ${n + 1}" style="flex:none;width:${s.step === n + 1 ? 20 : 7}px;height:7px;min-width:0 !important;min-height:0 !important;max-height:7px !important;padding:0 !important;border-radius:3.5px;border:none;cursor:pointer;background:${s.step === n + 1 ? C.accent : C.border};"></button>`).join('') + '</div>';
    }
    const prevDisabled = (s.phase === 'guide' && s.step === 1) || s.phase === 'ready';
    return `<div class="odv-footer" data-od-id="od-footer" style="display:flex;align-items:center;justify-content:space-between;padding:14px 24px;border-top:1px solid ${C.border};"><button data-act="back" style="flex:none;display:flex;align-items:center;gap:7px;height:40px;padding:0 16px;border-radius:10px;border:1px solid ${C.border};background:transparent;color:${C.dim};font-size:13.5px;font-weight:500;cursor:pointer;${prevDisabled ? 'opacity:.35;pointer-events:none;' : ''}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 6l-6 6 6 6"/></svg>Atrás</button><div class="odv-footer-center" style="display:flex;align-items:center;gap:12px;">${dots}<span data-od-hint style="font-size:12px;color:${C.faint};">${esc(hint || '')}</span></div><button data-act="next" data-od-next style="flex:none;display:flex;align-items:center;gap:7px;height:40px;padding:0 20px;border-radius:10px;border:none;background:${C.accent};color:${C.onAccent};font-size:13.5px;font-weight:600;cursor:pointer;${canAdvance(s) ? '' : 'opacity:.4;pointer-events:none;'}">${esc(nextLabel)}${last ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.5l4.5 4.5L19 7"/></svg>' : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg>'}</button></div>`;
}
...
function demoPanel(s) {
    ...
    return `<div data-od-id="od-demo-${s.step}" class="odv-demo-panel odv-demo-step-${s.step}" style="position:relative;border-left:1px solid ${C.border};background:${C.bg};display:flex;align-items:center;justify-content:center;padding:32px;min-height:280px;overflow:hidden;">${inner}</div>`;
}
export function renderOnboarding(s, chrome = '') {
    ...
    return `<div class="od-wrap odv-wrap" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:28px;background:${C.bg};color:${C.text};"><div class="odv-card" style="position:relative;width:100%;max-width:980px;background:${C.panel};border:1px solid ${C.border};border-radius:22px;overflow:hidden;display:flex;flex-direction:column;">${chrome}${topbar(stepCounter, s.phase === 'guide' && s.step < STEPS.length, progress)}${body}${footer(s, s.phase === 'guide', hint, nextLabel, s.phase === 'ready')}</div></div>`;
}
const ACTIONS = {
    next: s => navNext(s),
    back: s => navBack(s),
