...
export function topbar(stepCounter, showSkip, progress = null, chrome = '') {
    const skip = showSkip ? `<button type="button" data-act="goLast" aria-label="Omitir guía y elegir inicio" style="height:44px;min-height:44px;padding:0 13px;border-radius:8px;border:none;background:transparent;color:${C.faint};cursor:pointer;font-size:12.5px;font-weight:500;">Omitir guía</button>` : '';
...
export function footer(s, showDots, hint, nextLabel, last) {
    ...
    let dots = '';
    if (showDots) {
        dots = '<div style="display:flex;align-items:center;gap:2px;">' + STEPS.map((st, n) =>
            `<button type="button" data-act="dot" data-n="${n + 1}" title="${esc(st.title)}" aria-label="Ir al paso ${n + 1} de ${STEPS.length}: ${esc(st.title)}" aria-current="${s.step === n + 1 ? 'step' : 'false'}" style="width:44px;height:44px;min-width:44px;min-height:44px;border-radius:8px;border:none;cursor:pointer;padding:0;background:transparent;display:flex;align-items:center;justify-content:center;"><span aria-hidden="true" style="display:block;width:${s.step === n + 1 ? 22 : 8}px;height:8px;border-radius:5px;background:${s.step === n + 1 ? C.accent : C.border};"></span></button>`).join('') + '</div>';
    }
    const prevDisabled = (s.phase === 'guide' && s.step === 1) || s.phase === 'project' || s.phase === 'ready';
    return `<div data-od-id="od-footer" style="display:flex;align-items:center;justify-content:space-between;padding:16px 26px;border-top:1px solid ${C.border};"><button type="button" data-act="back" aria-label="Volver al paso anterior" aria-disabled="${prevDisabled ? 'true' : 'false'}" style="display:flex;align-items:center;gap:7px;height:44px;min-height:44px;padding:0 16px;border-radius:10px;border:1px solid ${C.border};background:transparent;color:${C.dim};font-size:13.5px;font-weight:500;cursor:pointer;${prevDisabled ? 'opacity:.35;pointer-events:none;' : ''}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 6l-6 6 6 6"/></svg>Atrás</button><div style="display:flex;align-items:center;gap:12px;">${dots}${hint ? `<span data-od-hint aria-live="polite" style="font-size:12px;color:${C.faint};">${esc(hint)}</span>` : ''}</div><button type="button" data-act="next" aria-label="${esc(nextLabel)}" aria-disabled="${canAdvance(s) ? 'false' : 'true'}" style="display:flex;align-items:center;gap:7px;height:44px;min-height:44px;padding:0 20px;border-radius:10px;border:none;background:${C.accent};color:${C.onAccent};font-size:13.5px;font-weight:600;cursor:pointer;${canAdvance(s) ? '' : 'opacity:.4;pointer-events:none;'}">${esc(nextLabel)}${last ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.5l4.5 4.5L19 7"/></svg>' : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg>'}</button></div>`;
}
...
export function choiceSection(s) {
    ...
    return `<div data-od-id="od-choice" style="min-height:434px;padding:38px 34px;display:flex;flex-direction:column;justify-content:center;max-width:620px;margin:0 auto;">${chip('Punto de partida')}<h1 style="margin:0;font-size:26px;font-weight:700;letter-spacing:-.015em;line-height:1.2;">¿Cómo quieres empezar?</h1><p style="margin:11px 0 24px;font-size:14.5px;color:${C.dim};line-height:1.55;">Configura la app desde cero o recupera datos que ya tengas guardados.</p><div role="radiogroup" aria-label="Opciones para empezar" style="display:grid;gap:11px;">${card('scratch', '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"/></svg>', 'Empezar desde cero', 'Siete pasos: empresa, proyecto, días de trabajo, jornada, posición y personal.')}${card('backup', '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M14 2v5h5"/><path d="M12 12.5V18"/><path d="m9.5 15 2.5-2.5 2.5 2.5"/></svg>', 'Cargar desde un backup', 'Restaura un archivo .json exportado desde esta app.')}${card('google', '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20 16.2A4.5 4.5 0 0 0 17.5 8h-1.8A7 7 0 1 0 4.4 15.3"/><path d="M12 11v7"/><path d="m8.5 14.5 3.5 3.5 3.5-3.5"/></svg>', 'Continuar con Google', 'Tu respaldo vive en tu cuenta: al iniciar sesión, la sincronización restaura tus datos automáticamente.')}${card('demo', '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="m10 8.5 5 3.5-5 3.5z"/></svg>', 'Explorar con datos de prueba', 'Carga una obra de ejemplo para probar la app sin registrar nada real.')}</div><div style="display:flex;justify-content:center;margin-top:18px;"><button type="button" data-act="skipOnboarding" aria-label="Saltar configuración inicial por ahora" style="height:44px;min-height:44px;padding:0 12px;border:none;background:transparent;color:${C.faint};font-family:inherit;font-size:12px;cursor:pointer;text-decoration:underline;text-underline-offset:3px;">Saltar por ahora</button></div>${s._choiceError ? `<div role="alert" aria-live="assertive" style="margin-top:16px;padding:12px 15px;border-radius:11px;border:1px solid #ef4444;background:rgba(239,68,68,.09);color:#fca5a5;font-size:13px;line-height:1.5;">${esc(s._choiceError)}</div>` : ''}</div>`;
}
export function projectSection(s) {
    const sourceLabel = s.source === 'google' ? 'Google Drive'
        : s.source === 'backup' ? 'Backup local'
            : s.source === 'demo' ? 'Datos de prueba' : 'Datos existentes';
    return `<div data-od-id="od-project" style="min-height:434px;padding:38px 34px;max-width:620px;margin:0 auto;width:100%;display:flex;flex-direction:column;justify-content:center;">
        ${chip('Proyecto de trabajo', 14)}
        <h1 style="margin:0;font-size:24px;font-weight:700;letter-spacing:-.012em;line-height:1.25;">Confirma el proyecto activo</h1>
        <p style="margin:10px 0 24px;font-size:14px;color:${C.dim};line-height:1.55;">Tus datos de ${esc(sourceLabel)} ya están disponibles. Confirma la obra o proyecto al que pertenecen para que personal, asistencia, nómina y Mini usen el mismo contexto.</p>
        <label style="${LBL}" for="f-project-confirm">Nombre del proyecto o de la obra</label>
        ${input('f-project-confirm', 'projectName', s.projectName, 'Ej: Torre Mirador', 'maxlength="80" aria-required="true" ')}
        <div style="display:flex;align-items:flex-start;gap:12px;margin-top:20px;padding:15px 16px;border-radius:12px;background:${C.panel2};">
            <span aria-hidden="true" style="width:40px;height:40px;flex:none;border-radius:11px;background:${C.accent};color:${C.onAccent};display:flex;align-items:center;justify-content:center;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V7l8-4 8 4v13"/><path d="M8 20v-5h8v5M8 9h.01M12 9h.01M16 9h.01"/></svg></span>
            <div><div style="font-size:13.5px;font-weight:700;">Un contexto para toda la app</div><div style="font-size:12.5px;color:${C.dim};line-height:1.5;margin-top:4px;">Si el respaldo ya contiene un proyecto activo, se propone su nombre. Puedes corregirlo aquí antes de entrar a la app.</div></div>
        </div>
        ${s._projectError ? `<div role="alert" aria-live="assertive" style="margin-top:16px;padding:12px 15px;border-radius:11px;background:color-mix(in oklch, ${C.bad} 20%, ${C.panel2});color:#fecaca;font-size:13px;line-height:1.5;">${esc(s._projectError)}</div>` : ''}
    </div>`;
}
...
export function setupSection(s) {
    ...
    if (su === 1) {
        b += `<label style="${LBL}" for="f-company">Nombre de la empresa</label>${input('f-company', 'company', s.company, 'Ej: Constructora Horizon S.R.L.', 'aria-required="true" ')}`;
    ...
    if (su === 2) {
        b += `<label style="${LBL}" for="f-project-name">Nombre del proyecto o de la obra</label>${input('f-project-name', 'projectName', s.projectName, 'Ej: Torre Mirador', 'maxlength="80" aria-required="true" ')}`;
        b += `<div style="display:flex;align-items:flex-start;gap:12px;margin-top:20px;padding:15px 16px;border-radius:12px;background:${C.panel2};"><span aria-hidden="true" style="width:40px;height:40px;flex:none;border-radius:11px;background:${C.accent};color:${C.onAccent};display:flex;align-items:center;justify-content:center;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V7l8-4 8 4v13"/><path d="M8 20v-5h8v5M8 9h.01M12 9h.01M16 9h.01"/></svg></span><div><div style="font-size:13.5px;font-weight:700;">Tu contexto de trabajo</div><div style="font-size:12.5px;color:${C.dim};line-height:1.5;margin-top:4px;">Este proyecto quedará activo al terminar. Personal, asistencia, nómina y transferencias con Mini usarán este contexto. Podrás crear otros proyectos después desde Configuración de proyectos.</div></div></div>`;
    }
    if (su === 3) {
    ...
    if (su === 4) {
        const presets = [8, 9, 10].map(h =>
            `<button type="button" data-act="hours" data-v="${h}" aria-pressed="${s.hours === h ? 'true' : 'false'}" aria-label="Jornada de ${h} horas por día" style="height:44px;min-height:44px;padding:0 15px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:600;${s.hours === h ? `background:${C.accent};color:${C.onAccent};border:none;` : `background:transparent;color:${C.dim};border:1px solid ${C.border};`}">${h}h</button>`).join('');
    ...
    if (su === 5) {
        const sw = POS_COLORS.map((c, n) =>
            `<button type="button" role="radio" aria-checked="${s.posColorIdx === n ? 'true' : 'false'}" data-act="swatch" data-n="${n}" title="Color ${n + 1}" aria-label="Color ${n + 1}" style="width:44px;height:44px;min-width:44px;min-height:44px;border-radius:9px;cursor:pointer;background:${c};border:2px solid ${s.posColorIdx === n ? C.text : 'transparent'};"></button>`).join('');
...
export function readySection(s) {
    ...
    const restored = s.readySummary || {};
    const restoredCompany = String(restored.company || s.company || '').trim() || '—';
    const restoredProject = String(restored.projectName || s.projectName || '').trim() || '—';
    const restoredEmployees = Number.isFinite(restored.employeeCount) ? restored.employeeCount : s.employees.length;
    const restoredPositions = Number.isFinite(restored.positionCount) ? restored.positionCount : null;
    const box = scratch
        ? `<div style="border:1px solid ${C.border};border-radius:14px;overflow:hidden;">${row('Empresa', s.company.trim() || '—')}${row('Proyecto', s.projectName.trim() || '—')}${row('Semana laboral', `${daysOn} días · ${DAY_LABELS.filter((_, n) => s.days[n]).join(' ')}`)}${row('Jornada', `${s.hours}h por día · ${daysOn * s.hours}h semanales`)}${row('Posiciones', s.posName.trim() ? s.posName.trim() + (s.posRate ? ` · $${s.posRate}/hr` : '') : '—')}${row('Personal', `${s.employees.length}${s.employees.length === 1 ? ' empleado' : ' empleados'}`)}${row('Respaldo', 'Solo en este dispositivo')}</div>`
        : `<div style="padding:12px 16px;border-radius:12px;background:${C.panel2};border:1px solid ${C.border};margin-bottom:12px;font-size:12.5px;color:${C.dim};">Los datos restaurados ya están disponibles en este dispositivo.</div><div style="border:1px solid ${C.border};border-radius:14px;overflow:hidden;">${row('Origen', s.source === 'google' ? 'Google Drive' : s.source === 'demo' ? 'Datos de prueba' : 'Archivo local')}${row('Empresa', restoredCompany)}${row('Proyecto', restoredProject)}${restoredPositions !== null ? row('Posiciones', String(restoredPositions)) : ''}${row('Personal', `${restoredEmployees}${restoredEmployees === 1 ? ' empleado' : ' empleados'}`)}</div>`;
...
export function renderOnboarding(s, chrome = '') {
    const stepCounter = s.phase === 'guide' ? `${s.step} / ${STEPS.length}`
        ...
        : s.phase === 'choice' ? 'Punto de partida'
            : s.phase === 'project' ? 'Proyecto' : 'Listo';
    ...
    const hint = s.phase === 'setup' && !canAdvance(s) ? (hintMap[s.setupStep] || '')
        : s.phase === 'project' && !canAdvance(s) ? 'Escribe el nombre del proyecto'
    ...
    const nextLabel = s.phase === 'guide' ? (s.step === STEPS.length ? 'Configurar la app' : 'Siguiente')
        ...
        : s.phase === 'project' ? 'Confirmar proyecto'
        : 'Continuar';
    ...
    const body = s.phase === 'guide' ? `<div class="odv-guide-grid" style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);min-height:434px;">${guideCopy(s)}${demo}</div>`
        ...
        : s.phase === 'project' ? projectSection(s)
        : readySection(s);
    const progress = (s.phase === 'guide' || s.phase === 'setup') ? { now: s.phase === 'guide' ? s.step : s.setupStep, max: s.phase === 'guide' ? STEPS.length : SETUP_TOTAL } : null;
    return `<div class="od-wrap odv-wrap" style="min-height:100vh;min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:28px;background:${C.bg};color:${C.text};"><div class="odv-card" style="position:relative;width:100%;max-width:980px;background:${C.panel};border:1px solid ${C.border};border-radius:22px;overflow:hidden;">${topbar(stepCounter, s.phase === 'guide' && s.step < STEPS.length, progress, chrome)}${body}${footer(s, s.phase === 'guide', hint, nextLabel, s.phase === 'ready')}</div></div>`;
}
const ACTIONS = {
    next: s => navNext(s),
    back: s => navBack(s),
