...
export class LegacyPayrollUnavailableError extends Error {
    constructor() {
        super('Legacy payroll calculation unavailable while projects are enabled; use calculateEmployeePayrollScoped()');
        this.name = 'LegacyPayrollUnavailableError';
        this.code = 'LEGACY_PAYROLL_UNAVAILABLE_WHEN_SCOPED';
    }
}
...
export async function resolveScopedPayrollContext(state, opts = {}) {
    ...
    try {
        const idb = opts.idb;
        config = idb
            ? await ProjectPayrollConfigStore.getConfig(capturedProjectId, { idb })
            : await ProjectPayrollConfigStore.getConfig(capturedProjectId);
    } catch (e) {
        fetchError = e;
    }
    if (!config) {
...
export class PayrollService {
    ...
    calculateEmployeePayrollScoped(
        empId,
        startDateKey,
        endDateKey,
        deductions = null,
        ...
        // resolveScopedPayrollContext captures the project synchronously before
        // its first await, so a later A → B switch cannot change this request.
        const contextPromise = resolveScopedPayrollContext(this.state);
        return contextPromise.then(({ mode, ctx, config }) => {
            if (mode !== 'scoped') {
                throw new Error('Scoped payroll calculation requires projects to be enabled with an active project');
            }
            return calculateEmployeePayrollScopedInternal(
                ctx,
                config,
                empId,
                startDateKey,
    ...
    // Legacy entry point remains strictly synchronous.
    calculateEmployeePayroll(
        ...
        if (isProjectsEnabled()) throw new LegacyPayrollUnavailableError();
