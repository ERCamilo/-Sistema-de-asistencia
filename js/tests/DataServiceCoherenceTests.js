...
testRunner.addSuite("DataService.loadAll — Coherencia explícita (Fase 4 Paso 3)", {
    ...
    "importa buildAttendanceIndex desde AppState"() {
        testRunner.assert(
    ...
    "tras cargar la asistencia, limpia statsCache.mtd mayoristamente"() {
        const body = loadAllBody();
        testRunner.assert(
            /statsCache\.mtd\s*=\s*\{\s*\}/.test(body),
            'loadAll debe limpiar state.statsCache.mtd = {} (las stats viejas son basura tras un restore)'
        );
    },
    ...
    "la coherencia vive DENTRO del bloque if (data.attendance) (no corre sin datos)"() {
        ...
        const coherenceIdx = body.search(/statsCache\.mtd\s*=\s*\{\s*\}/);
        testRunner.assert(guardIdx !== -1, 'debe existir el guard if (data.attendance)');
        testRunner.assert(
            coherenceIdx > guardIdx,
            'la limpieza de statsCache debe estar después del guard if (data.attendance), no en el camino sin asistencia'
        );
    }
});
