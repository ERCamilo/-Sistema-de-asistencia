...
describe('A3 PayrollService/Period scoped — frozen contract', () => {
    ...
    test('ON: legacy calculation fails synchronously and points callers to the scoped API', () => {
        setProjectsEnabled(true);
        replaceEntityScope({ enabled: true, projectId: PRJ_A, defaultProjectId: PRJ_DEFAULT });
        const svc = new PayrollService(makePayrollState());

        expect(() => svc.calculateEmployeePayroll('E-A', '2026-08-24', '2026-08-24'))
            .toThrow(/calculateEmployeePayrollScoped/);
    ...
    test('ON + missing config → fail closed with explicit "config unavailable" error (no fallback)', async () => {
        ...
        await expect(svc.calculateEmployeePayrollScoped('E-A', '2026-08-24', '2026-08-24')).rejects.toThrow(/config unavailable/i);
    ...
    test('Operation started in A → await → UI switches to B → result stays fully A (snapshot stable)', async () => {
        ...
        replaceEntityScope({ enabled: true, projectId: PRJ_A, defaultProjectId: PRJ_DEFAULT });
        const svc = new PayrollService(state);
        const promiseA = svc.calculateEmployeePayrollScoped('E-A', '2026-08-24', '2026-08-24'); // captures A before await
        ...
        const promiseB = svc.calculateEmployeePayrollScoped('E-B', '2026-08-24', '2026-08-24');
