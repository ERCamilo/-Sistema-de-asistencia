...
**Actualización F1.6 (2026-08-30 — cierre B2 2026-08-31):** **A0–A6 ✅ están aprobados, Tanda A ✅ está completa, B1 ✅ y B2 ✅ están cerradas y aprobadas formalmente (hook infra `0fc54df`; B2.0 `580227a`, B2.1 `114f041`+MC1 `c9da41e`, B2.2 `4734403`+MC1 `d563055`, B2.3 `5c3e419`+MC1 `64421a4`, B2.4/cierre `fd8dc7e` → árbol final `d5a6f8e6aa53a9ce318781bcc82eabb2763b12f0`; suite 374/3620 con flake caveat `ProjectStoreRealIdb` 1ms retry verde no bloqueante; resolución B2.4 OFF→new legacy→ON reentry).** `EXC-REVIEW-B1-001` fue aceptada solo para la publicación B1 y no se traslada. **B3 🟢 autorizada exclusivamente para repositorio/sync cloud por proyecto (lazy promotion preservando id/fingerprint/supersedesId, native vs promoted-legacy, sin rediseño de identidad económica); B4–B5 🔒 y F1.7 🔒.**
...
**Orden ejecutado de A6 + MC1 (✅ cerrado y aprobado formalmente sobre `b286d70` + `f735dd6`):**
...
**Entrega B1 (✅ cerrada y aprobada formalmente):** `193273e` implementa schema 3 para construcción explícita con proyectos ON, `projectId` obligatorio y semánticamente inmutable, identidad/fingerprint project-aware, correcciones deterministas y void owner-preserving. OFF conserva schema 2 exacto sin `projectId` ni dual-write. Evidencia 41/41, 129/129 y 3586/3586; `R3-001` fue corregido en 15 líneas. `EXC-REVIEW-B1-001` aplica solo a esta publicación.

**Entrega B2 (✅ cerrada y aprobada formalmente — F1.6-B2):** hook infra `0fc54df` + B2.0 `580227a` + B2.1 `114f041`+MC1 `c9da41e` + B2.2 `4734403`+MC1 `d563055` + B2.3 `5c3e419`+MC1 `64421a4` + B2.4/cierre `fd8dc7e` → `d5a6f8e6aa53a9ce318781bcc82eabb2763b12f0`. Persistencia local project-scoped de cierres: store/consultas/cachés/historial/paginación/búsqueda por período/`getById`/sync state y migración `projectId`-aware; stamper schema 2→3 reanudable/idempotente por lotes pequeños; promoción legacy schema 2 por metadata al default sin reescritura económica; resolución B2.4 OFF→new legacy→ON reentry (legacy nuevo creado en OFF migra limpiamente al reentrar ON). Suite 374/3620 con un flake no bloqueante `ProjectStoreRealIdb` (`Expected: 1 Received: undefined` `toBe` 1ms, producción no modificada por B2.4, retry verde).

**Alcance B2 cerrado (✅ `fd8dc7e`/`d5a6f8e`):** almacenamiento local de cierres realmente scoped por proyecto; aislamiento obligatorio por `projectId` en store, consultas, cachés, historial, paginación, búsqueda por período, `getById`, estado de sync y migración; stamper schema 2→3 reanudable, idempotente y por lotes pequeños; promoción legacy schema 2 por metadata al proyecto predeterminado bajo contrato de migración, sin reescrituras económicas históricas indiscriminadas; resolución OFF→new legacy→ON reentry. Las pruebas demuestran que A no observa ni altera B, aun con períodos idénticos (374/3620 con flake `ProjectStoreRealIdb` 1ms retry verde). Quedan excluidos B3 🟢 cloud/sync cierres, B4 préstamos/ajustes/cuotas/`markAsPaid`, B5 PDF/JSON/SplitX/recuperación y F1.7. Gentle AI volvió al protocolo ordinario tras B1.

**Alcance B3 autorizado exclusivamente (próxima unidad):** repositorio/sync cloud project-scoped de cierres con promoción lazy preservando `id`/`fingerprint`/`supersedesId`, contrato native vs promoted-legacy, sin rediseño de identidad económica. Ver [`F1.6-B2-bitacora.md`](../fase-1/F1.6-B2-bitacora.md).
...
| ADR-015 | **Implementada, cerrada y aprobada en B1 `193273e` y B2 `fd8dc7e`/`d5a6f8e`:** con proyectos ON los cierres nuevos usan schema 3 con `projectId` semánticamente inmutable e identidades/fingerprints/IDs project-aware; OFF conserva schema 2 exacto sin `projectId` ni dual-write. B2 ✅ cierra persistencia local y migración project-scoped (B2.0–B2.4, hook `0fc54df`, 374/3620, OFF→new legacy→ON reentry; flake `ProjectStoreRealIdb` 1ms retry verde); B3 🟢 autoriza repositorio/sync cloud por proyecto con lazy promotion preservando id/fingerprint/supersedesId (native vs promoted-legacy); B4–B5 permanecen 🔒 | Todo cierre, ajuste, préstamo, exportación y recuperación debe tener propietario económico inequívoco | SA, Integración | v0.5 (2026-08-30) + B2 cierre 2026-08-31 |
...
Para el estado vigente de F1.6, esa orden histórica ya fue completada. La orden actual, actualizada el 2026-08-31 tras el cierre B2, es:

> **Estado exacto: A0–A6 ✅; Tanda A ✅ completa; B1 ✅; B2 ✅ cerrada y aprobada formalmente (`fd8dc7e`/`d5a6f8e`, hook `0fc54df`, 374/3620 con flake `ProjectStoreRealIdb` 1ms retry verde; B2.4 OFF→new legacy→ON reentry) · B3 🟢 autorizada exclusivamente para repositorio/sync cloud por proyecto (lazy promotion preservando id/fingerprint/supersedesId, native vs promoted-legacy) · B4–B5 🔒 · F1.7 🔒.** Ejecutar únicamente B3 dentro de su alcance cloud/sync por proyecto con promoción lazy y bajo protocolo ordinario.
...
| F1 Contexto de proyecto | En ejecución — F1.5 cerrada; F1.6: **A0–A6 ✅ · Tanda A ✅ completa · B1 ✅ · B2 ✅ cerrada y aprobada formalmente (`fd8dc7e`/`d5a6f8e`, hook `0fc54df`, 374/3620 con flake caveat) · B3 🟢 autorizada exclusivamente cloud/sync cierres · B4–B5 🔒 · F1.7 🔒** | SA | **Ejecutar solo B3** |
