...
**Actualización F1.6 (2026-08-28 — A1 PayrollProjectContext implementado y técnicamente validado, pendiente revisión formal; DEP-SA-004 ✅ Cerrado y aprobado 2026-08-27):** la arquitectura de nómina multiproyecto está aprobada con la modificación bloqueante DEP-SA-004. **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato congelado `explicit > default autoritativo > legacy-unresolved:*`); **A0.5 / DEP-SA-004 está ✅ Cerrado y aprobado por Dirección el 2026-08-27** — `51a7611` + `50343ee` (20/20, 360/360 · 3485 · 86s, ALLOW 0 findings; MC1+MC2 resueltos — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) — `activeProjectId` local, `projectId` canónico account-level. **A1 — PayrollProjectContext está 🟡 Implementado y técnicamente validado el 2026-08-28** (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings — ver [`docs/fase-1/F1.6-A1-bitacora.md`](docs/fase-1/F1.6-A1-bitacora.md) §3) — **pendiente revisión formal**; sin wiring, sin `projectPayrollConfigs`; **A2–A6 y F1.7 permanecen bloqueadas;** F1.6 no está completa más allá de A1. Ver [`docs/fase-1/F1.6-A1-bitacora.md`](docs/fase-1/F1.6-A1-bitacora.md).
...
| DEP-SA-004 | SA | SA (valida Dirección) | F1.6-A2–A6, estado económico durable por `projectId` más allá de A1 y F1.6-B cloud | Identidad canónica de `Project` entre dispositivos: la misma cuenta/obra debe compartir un único `projectId`; `activeProjectId` sigue siendo local. Registro, adopción/promoción y protección contra carreras mínimos y SA-only; no incluye organizaciones, roles, membresías, Mini ni Integración | Bloqueante para A2–A6 | **✅ Cerrado y aprobado por Dirección el 2026-08-27** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1 purga `asistencia_canonical_*`/`asistencia_alias_*` + MC2 `pending\|\|dead` — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) sobre **A0 ✅ Cerrado y aprobado (c7a9e0c + 333a516; 354/354 · 3409 PASS)**; **A1 🟡 Implementado y técnicamente validado el 2026-08-28 (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings) — **pendiente revisión formal**; A2–A6 bloqueadas** — DEP-SA-004 cerrado |
...
Para el estado vigente de F1.6, esa orden histórica ya fue completada. La orden actual (actualizada 2026-08-28 — A1 PayrollProjectContext implementado y técnicamente validado el 2026-08-28, pendiente revisión formal; DEP-SA-004 ✅ Cerrado y aprobado 2026-08-27) es:

> **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`). **A0.5 / DEP-SA-004 está ✅ Cerrado y aprobado por Dirección el 2026-08-27** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1 purga prefijo + MC2 `dead` canonicalization — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) — `activeProjectId` local, `projectId` canónico account-level; SA-only, registro/adopción/promoción + guard carreras. **A1 — PayrollProjectContext está 🟡 Implementado y técnicamente validado el 2026-08-28** (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings) — **pendiente revisión formal**; sin wiring, sin `projectPayrollConfigs`. No iniciar A2–A6, estado económico durable por `projectId` más allá de A1, cloud de F1.6-B ni F1.7 (A2–A6 bloqueadas hasta cierre formal de A1).
...
| F1 Contexto de proyecto | En ejecución — **F1.5 cerrada y aprobada** (354/354 suites, 3401 tests, 0 fallos); F1.6 con arquitectura aprobada y modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409 PASS, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 ✅ Cerrado y aprobado por Dirección el 2026-08-27** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1+MC2 resueltos §3.1; DEP-SA-004 cerrado) — `activeProjectId` local, `projectId` canónico; **A1 🟡 Implementado y técnicamente validado el 2026-08-28** (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings) — **pendiente revisión formal**; **A2–A6 y F1.7 bloqueadas** | SA | **A1 🟡 Implementado y técnicamente validado el 2026-08-28** (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings) — **pendiente revisión formal de Dirección (habilita A2)**; A2–A6 y estado económico durable o cloud F1.6-B bloqueados hasta cierre formal de A1 (DEP-SA-004 permanece cerrado) |
