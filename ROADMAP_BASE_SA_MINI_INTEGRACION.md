...
| F3.P2P-5 | SA + Mini | Centro de actividad P2P accionable separado de la portada de conexión | 0% (supplemental / non-weighted) | Vista/overlay de pendientes y eventos que permiten actuar: revisar, abrir, reintentar, descartar o inspeccionar error; el store interno sigue alimentando badges y trazabilidad | La portada no muestra logs pasivos; sólo los pendientes reales generan badge; cada elemento visible del centro tiene una acción útil o contexto de error; no crea autoridad ni writers P2P nuevos | F3.P2P-1–F3.P2P-4; reutiliza inbox/review canónicos de F3.6/F3.10 y puede ampliarse con F9 | `p2p-action-center` |

**F3.P2P-5 — regla de producto:** la pantalla principal de Transferencias se mantiene enfocada en estado de conexión, dispositivos y acciones directas. El historial/log técnico no ocupa espacio permanente. `P2PActivityStore` puede conservar metadata local para badge, deduplicación, diagnóstico y futura trazabilidad, pero la UI `Actividad` sólo se materializa cuando sus elementos son accionables (`Revisar`, `Abrir`, `Reintentar`, `Descartar` o inspeccionar un error). Los eventos puramente informativos permanecen fuera de la portada. Esta unidad es futura, no bloquea F3.4 y no modifica los contratos ni pesos de F3.
