...
**Actualización F1.6 (2026-08-26 — cierre formal Dirección):** la arquitectura de nómina multiproyecto está aprobada con la modificación bloqueante DEP-SA-004. **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato congelado `explicit > default autoritativo > legacy-unresolved:*`); **A0.5 / DEP-SA-004 está 🟢 Autorizado / en ejecución (exclusivamente)** — `activeProjectId` local, `projectId` canónico account-level. **A1–A6 y F1.7 permanecen bloqueadas hasta validar A0.5**; F1.6 no está completa y no hay comportamiento funcional de nómina aceptado.
...
Para el estado vigente de F1.6, esa orden histórica ya fue completada. La orden actual (actualizada 2026-08-26 — tras cierre formal Dirección) es:

> **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`). **A0.5 / DEP-SA-004 está 🟢 Autorizado / en ejecución exclusivamente** (`activeProjectId` local, `projectId` canónico account-level; SA-only, registro/adopción/promoción + guard contra carreras). No iniciar A1–A6, estado económico durable por `projectId`, cloud de F1.6-B ni F1.7 hasta validar la identidad canónica entre dispositivos.
...
| F1 Contexto de proyecto | En ejecución — **F1.5 cerrada y aprobada** (354/354 suites, 3401 tests, 0 fallos); F1.6 con arquitectura aprobada y modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 🟢 Autorizado / en ejecución (exclusivamente)** — `activeProjectId` local, `projectId` canónico; **A1–A6 y F1.7 bloqueadas hasta validar A0.5** | SA | Validar A0.5 antes de A1–A6, estado económico durable o cloud F1.6-B (A0 ya cerrado 2026-08-26) |
