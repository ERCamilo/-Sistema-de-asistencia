...
**Actualización F1.6 (2026-08-27 — implementación A0.5 + micro-cierre `50343ee`, pendiente de cierre formal):** la arquitectura de nómina multiproyecto está aprobada con la modificación bloqueante DEP-SA-004. **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato congelado `explicit > default autoritativo > legacy-unresolved:*`); **A0.5 / DEP-SA-004 está ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal de Dirección** — base `51a7611` (ProjectRegistry 19 líneas, ProjectAdoption 22 líneas, boot `uid`, G1–G5; 18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) + micro-cierre `50343ee` (4 archivos, 117 líneas: `LocalWipeService` purga prefijo `asistencia_canonical_*`/`asistencia_alias_*` + `ProjectAdoption` `pending||dead` + `MainSyncStore` `bumpGeneration`; 20/20, 360/360 · 3485 · 86s, ALLOW 0 findings; MC1+MC2 resueltos — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) — `activeProjectId` local, `projectId` canónico account-level. **A1–A6 y F1.7 permanecen bloqueadas hasta cierre formal de A0.5**; F1.6 no está completa y no hay comportamiento funcional de nómina formalmente aprobado. Ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md).
...
| DEP-SA-004 | SA | SA (valida Dirección) | F1.6-A1–A6, estado económico durable por `projectId` y F1.6-B cloud | Identidad canónica de `Project` entre dispositivos: la misma cuenta/obra debe compartir un único `projectId`; `activeProjectId` sigue siendo local. Registro, adopción/promoción y protección contra carreras mínimos y SA-only; no incluye organizaciones, roles, membresías, Mini ni Integración | Bloqueante | **✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal de Dirección** — base `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) + micro-cierre `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1 purga `asistencia_canonical_*`/`asistencia_alias_*` + MC2 `pending\|\|dead` — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) sobre **A0 ✅ Cerrado y aprobado (c7a9e0c + 333a516; 354/354 · 3409 PASS)**; **A1–A6 bloqueadas hasta cierre formal de A0.5 — DEP-SA-004 NO se cierra hasta veredicto formal (no presentar como aprobado)** — ver §3 + §3.1 |
...
Para el estado vigente de F1.6, esa orden histórica ya fue completada. La orden actual (actualizada 2026-08-27 — tras micro-cierre `50343ee`, pendiente de cierre formal) es:

> **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`). **A0.5 / DEP-SA-004 está ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal de Dirección** — base `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) + micro-cierre `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1 purga prefijo + MC2 `dead` canonicalization — ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md) §3.1) — `activeProjectId` local, `projectId` canónico account-level; SA-only, registro/adopción/promoción + guard carreras. No iniciar A1–A6, estado económico durable por `projectId`, cloud de F1.6-B ni F1.7 hasta cierre formal de la identidad canónica (DEP-SA-004 no se cierra hasta veredicto formal).
...
| F1 Contexto de proyecto | En ejecución — **F1.5 cerrada y aprobada** (354/354 suites, 3401 tests, 0 fallos); F1.6 con arquitectura aprobada y modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409 PASS, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal** — `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1+MC2 resueltos §3.1) — `activeProjectId` local, `projectId` canónico; **A1–A6 y F1.7 bloqueadas hasta cierre formal de A0.5** | SA | **Cierre formal de A0.5 antes de A1–A6**, estado económico durable o cloud F1.6-B (A0 ya cerrado 2026-08-26; A0.5 + micro-cierre ya implementado 2026-08-27, pendiente solo veredicto formal) |
