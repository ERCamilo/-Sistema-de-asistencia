...
**Actualización F1.6 (2026-08-27 — implementación técnica A0.5, pendiente de revisión formal):** la arquitectura de nómina multiproyecto está aprobada con la modificación bloqueante DEP-SA-004. **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato congelado `explicit > default autoritativo > legacy-unresolved:*`); **A0.5 / DEP-SA-004 está ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (ProjectRegistry 19 líneas, ProjectAdoption 22 líneas, boot `uid`, G1–G5; 18/18, 360/360 · 3483 · 86s, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico account-level. **A1–A6 y F1.7 permanecen bloqueadas hasta revisión formal de A0.5**; F1.6 no está completa y no hay comportamiento funcional de nómina formalmente aprobado. Ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md).
...
Para el estado vigente de F1.6, esa orden histórica ya fue completada. La orden actual (actualizada 2026-08-27 — tras implementación técnica A0.5, pendiente de revisión formal) es:

> **A0 está ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, F1.4+A0 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`). **A0.5 / DEP-SA-004 está ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico account-level; SA-only, registro/adopción/promoción + guard contra carreras (ver [`docs/fase-1/F1.6-A0.5-bitacora.md`](docs/fase-1/F1.6-A0.5-bitacora.md)). No iniciar A1–A6, estado económico durable por `projectId`, cloud de F1.6-B ni F1.7 hasta aprobación formal de la identidad canónica.
...
| F1 Contexto de proyecto | En ejecución — **F1.5 cerrada y aprobada** (354/354 suites, 3401 tests, 0 fallos); F1.6 con arquitectura aprobada y modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409 PASS, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico; **A1–A6 y F1.7 bloqueadas hasta aprobación formal de A0.5** | SA | **Revisión formal de A0.5 antes de A1–A6**, estado económico durable o cloud F1.6-B (A0 ya cerrado 2026-08-26; A0.5 ya implementado 2026-08-27) |
