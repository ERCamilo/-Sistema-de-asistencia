...
| Estado | **F0 100%; F1.1–F1.5 cerradas** · F1.6 con arquitectura aprobada · **A0–A5 ✅ cerrados y aprobados por Dirección 2026-08-29** (`6c1cb2c` A4 + `ae66121` A5; A5: 17/17, 56/56, 366/366 · 3542, ALLOW · 0 findings tras fix WARNING `Object.assign`) · **A6 🟢 autorizado exclusivamente (cierre de Tanda A) · B1–B5 🔒 · F1.7 🔒** · DEP-SA-004 cerrado |
| Punto de parada | **A5 ✅ cerrado y aprobado formalmente por Dirección 2026-08-29 sobre `ae66121`** (helper 24 líneas, 8 archivos +243/-4, WARNING inicial resuelto vía `Object.assign`) — **A6 🟢 autorizado exclusivamente (cierre de Tanda A)**; **B1–B5 🔒 · F1.7 🔒**. DEP-SA-004 permanece cerrado. |
| Rama de trabajo | `feature/factor-dias-no-laborables` · commit de referencia `ae66121` (A5) / `6c1cb2c` (A4) · árbol limpio antes de esta reconciliación documental |
...
| **F1.6-A5 H-05 exportConfig (transitorio)** | [`F1.6-A5-bitacora.md`](F1.6-A5-bitacora.md) | ✅ **Cerrado y aprobado formalmente por Dirección 2026-08-29** — `ae66121` (8 archivos, +243/-4; helper 24 líneas; 17/17, 56/56, 366/366 · 3542, ALLOW · 0 findings tras fix de WARNING `Object.assign` vía `Object.assign` shallow copy) — helper `ExportConfigSanitizer` elimina solo `exportConfig`, preserva `payrollDefaults`/`projectPayrollConfigs`; sanea mirror/replace/snapshot + `getSnapshot`/`getFullState` + `PersistenceService` `_mirrorSnapshot` + `DataOps` frozen/`cleanCloud` + `app.js` `applyRemoteData`/`applyBackupData`/`prepareRestoredState`; verifica no resurrección tras sync/restore/snapshot con A→B→A y OFF paridad preservadas | **A5 ✅ cerrado — A6 🟢 autorizado (cierre de Tanda A)**; B1–B5 🔒, F1.7 🔒 |
| **F1.6-A6 Cierre de Tanda A — gates + matriz consolidada** | `F1.6-nomina-multiproyecto.md` §5 + Roadmap §7/§15 | 🟢 **Autorizado exclusivamente — cierre de Tanda A** — con proyectos ON bloquear efectivamente: cierres de nómina, ajustes económicos persistidos/programados, operaciones de préstamo, historial económico, pago definitivo, exportaciones finales (no solo botones ocultos en `ScopedPayrollTab`, también llamadas directas programáticas deben fallar explícita y seguramente sin mutaciones parciales; OFF preserva legacy) + matriz consolidada A/B de toda la Tanda A (mismo `#12` `employeeId` distinto, configs/períodos/feriados/horas distintos, asistencia aislada, A→B→A, async A mientras se cambia a B, H-05 sin resurrección de `exportConfig`, `buildAttendanceIndex` RAW detrás de fronteras scoped). No implementar `PayrollClosure` schema3, persistencia económica por proyecto, migración de cierres, economic cloud, préstamos project-aware, PDF/SplitX | **A6 es el siguiente paso**; B1–B5 🔒, F1.7 🔒 |
...
- **A5 evidencia** → 17/17 (6 helper + 11 wiring), 56/56 agrupada, 366/366 · 3542, ALLOW · 0 findings tras corrección de WARNING (live-reference mutation → `Object.assign`); **A6 🟢 autorizado — cierre de Tanda A**
- **A6 cierre de Tanda A (🟢 autorizado)** → `F1.6-nomina-multiproyecto.md` §5 + Roadmap §7/§15 — gates con proyectos ON bloquean efectivamente cierres/ajustes persistidos-programados/préstamos/historial/pago definitivo/exportaciones finales (no solo botones ocultos en `ScopedPayrollTab`, también llamadas directas programáticas fallan explícita y seguramente sin mutaciones parciales; OFF preserva legacy) + matriz consolidada A/B de toda la Tanda A; no implementar `PayrollClosure` schema3, persistencia económica por proyecto, migración, economic cloud, préstamos project-aware, PDF/SplitX

## Contrato A4 — ✅ Cerrado y aprobado formalmente por Dirección 2026-08-28 · A5 ✅ cerrado 2026-08-29

> A4 fue ejecutado exactamente en el orden congelado siguiente y quedó **cerrado y aprobado formalmente por Dirección 2026-08-28** sobre `6c1cb2c` (12/12, 49/49, 365/365 · 3525, ALLOW · 0 findings). A4 ✅ y A5 ✅ forman la base para **A6 🟢 autorizado exclusivamente (cierre de Tanda A)**.
...
1. Leer roadmap §12–§16, este índice, [`F1.6-nomina-multiproyecto.md`](F1.6-nomina-multiproyecto.md) y las bitácoras evaluables A0–A5, especialmente [`F1.6-A5-bitacora.md`](F1.6-A5-bitacora.md) como cierre formal A5 ✅ y [`F1.6-A4-bitacora.md`](F1.6-A4-bitacora.md) como cierre formal de A4.
2. Estado exacto: **A0–A5 ✅ cerrados y aprobados por Dirección 2026-08-29 (A4 `6c1cb2c` + A5 `ae66121`; A5: 17/17, 56/56, 366/366 · 3542, ALLOW · 0 findings tras fix WARNING `Object.assign`) · A6 🟢 autorizado exclusivamente (cierre de Tanda A) · B1–B5 🔒 · F1.7 🔒**. DEP-SA-004 cerrado.
3. Próxima acción: **ejecutar exclusivamente A6 🟢 (cierre de Tanda A)** — gates con proyectos ON bloqueen efectivamente cierres/ajustes persistidos-programados/préstamos/historial/pago definitivo/exportaciones finales (no solo botones ocultos en `ScopedPayrollTab`, también llamadas directas programáticas deben fallar explícita y seguramente sin mutaciones parciales; OFF preserva legacy) + matriz consolidada A/B de toda la Tanda A (mismo `#12` `employeeId` distinto, configs/períodos/feriados/horas distintos, asistencia aislada, A→B→A, async A mientras se cambia a B, H-05 sin resurrección de `exportConfig`, `buildAttendanceIndex` RAW detrás de fronteras scoped). No implementar `PayrollClosure` schema3, persistencia económica por proyecto, migración de cierres, economic cloud, préstamos project-aware, PDF/SplitX. No tocar `PayrollClosure`/closures/loans/economic adjustments/PDF/SplitX/economic cloud/petty cash fuera del contrato A6. **B1–B5 🔒 y F1.7 🔒**. Ver [`F1.6-A5-bitacora.md`](F1.6-A5-bitacora.md) §5/§8 y `F1.6-nomina-multiproyecto.md` §5 + Roadmap §7/§15.
