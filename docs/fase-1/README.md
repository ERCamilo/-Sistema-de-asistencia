...
| **F1.6-A0.5 Bitácora identidad canónica** | [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) | ✅ **Cerrado y aprobado por Dirección 2026-08-27** — `51a7611` + `50343ee` (ProjectRegistry 19 líneas, ProjectAdoption 22 líneas + filtro `dead`, `LocalWipeService` purga prefijo, `MainSyncStore` `bumpGeneration`; 20/20 + 360/360 · 3485 · 86s, ALLOW 0 findings; MC1+MC2 resueltos; DEP-SA-004 cerrado); frontera rollback `51a7611` + `50343ee` | Lectura obligatoria — **A0.5 cerrado; A1 ✅ cerrado; A2 ✅ cerrado y aprobado; A3 🟢 autorizado** |
| **F1.6-A1 Bitácora PayrollProjectContext** | [`F1.6-A1-bitacora.md`](F1.6-A1-bitacora.md) | ✅ **Cerrado y aprobado por Dirección el 2026-08-28** — `185e1cd` (PayrollProjectContext 160 líneas, 10/10 + 55/55 enfocado · 361/361 · 3495 · 0 fallos, ALLOW 0 findings; captura antes de `await`, OFF paridad exacta, freeze, #12 por `employeeId`, attendance snapshot, `buildAttendanceIndex` intacto, sin wiring, sin `projectPayrollConfigs`); frontera rollback `185e1cd` | Lectura obligatoria — **A1 ✅ cerrado; A2 ✅ cerrado y aprobado 2026-08-28; A3 🟢 autorizado** |
| **F1.6-A2 Bitácora projectPayrollConfigs** | [`F1.6-A2-bitacora.md`](F1.6-A2-bitacora.md) | ✅ **Cerrado y aprobado por Dirección el 2026-08-28** — `d5858dc` (modelo 133 líneas + store 119 líneas + IDB v18 + boot seed 18 líneas + 221 líneas de tests; 7/7 + 62/62 agrupada · 362/362 · 3502 · 0 fallos, ALLOW 0 findings; store `projectPayrollConfigs` por `projectId`, 10 campos versionados, semilla atómica idempotente, flag OFF sin dual-write, A/B isolation, reload, canónico vs active, local-only sin cloud/wiring); frontera rollback `d5858dc` — **veredicto formal 2026-08-28** | Lectura obligatoria — **A2 ✅ cerrado y aprobado; A3 🟢 Autorizado — PayrollService/Period scoped** |
...
1. Leer roadmap §12–§16, este índice, [`F1.6-nomina-multiproyecto.md`](F1.6-nomina-multiproyecto.md) y las bitácoras evaluables [`F1.6-A0-bitacora.md`](F1.6-A0-bitacora.md), [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md), [`F1.6-A1-bitacora.md`](F1.6-A1-bitacora.md) y [`F1.6-A2-bitacora.md`](F1.6-A2-bitacora.md).
2. Estado: F0 al 100%; F1.1–F1.5 cerradas; **F1.6 con arquitectura aprobada con modificación DEP-SA-004, A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, 76 acotados, 354/354 · 3409, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`) y **A0.5 / DEP-SA-004 ✅ Cerrado y aprobado por Dirección el 2026-08-27** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1+MC2 resueltos §3.1; DEP-SA-004 cerrado) — `activeProjectId` local, `projectId` canónico account-level; **A1 ✅ Cerrado y aprobado por Dirección el 2026-08-28** (`185e1cd`; 10/10, 55/55 · 361/361 · 3495, ALLOW 0 findings); **A2 ✅ Cerrado y aprobado por Dirección el 2026-08-28** (`d5858dc`; 7/7, 62/62 agrupada, 362/362 · 3502, ALLOW 0 findings); **A3 🟢 Autorizado — PayrollService/Period scoped**; **A4–A6 y F1.7 bloqueadas**.
3. Próxima acción: **A3 — PayrollService/Period scoped** (ver [`F1.6-A2-bitacora.md`](F1.6-A2-bitacora.md) §5 y [`F1.6-nomina-multiproyecto.md`](F1.6-nomina-multiproyecto.md) §5 — `d5858dc`; 7/7, 62/62 agrupada, 362/362 · 3502, ALLOW 0 findings; local-only). Mantener **A4–A6 y F1.7 bloqueadas** hasta avance ordenado. **Condiciones para A3:** sin fallback silencioso a settings globales cuando flag ON y config es `null`; snapshot shallow debe probarse con mutación in-place de propiedades. **A0, A0.5, A1 y A2 ya están cerrados — no requieren nueva revisión.**
