...
| Estado | **F0 100%; F1.1–F1.5 cerradas** · arquitectura F1.6 aprobada con modificación DEP-SA-004 · **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (ProjectIdentityA05 18/18, 360/360 · 3483 · 0 fallos, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico account-level · **A1–A6 y F1.7 bloqueadas hasta revisión formal de A0.5** |
| Punto de parada | F1.5: **354/354 suites · 3401 tests · 0 fallos** (cerrada y aprobada). Post-A0: **354/354 suites · 3409 tests · 0 fallos · 189.372 s · sin reintentos** — **A0 ✅ Cerrado y aprobado por Dirección 2026-08-26** (RecordKey 13/13, F1.4+A0 12/12, 76 acotados, fresh ALLOW); Post-A0.5: **360/360 suites · 3483 tests · 0 fallos · 86s** — **A0.5 ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (ProjectIdentityA05 18/18, ALLOW con 2 WARNINGS); **A1–A6 y F1.7 bloqueadas hasta revisión formal de A0.5** |
...
| **F1.6 Nómina multiproyecto** | [`F1.6-nomina-multiproyecto.md`](F1.6-nomina-multiproyecto.md) | Arquitectura aprobada con modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico; **A1–A6 bloqueadas hasta revisión formal de A0.5** | F1.7 bloqueada hasta cierre F1.6 (A0.5 → A1–A6) |
...
| **F1.6-A0.5 Bitácora identidad canónica** | [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) | ✅ **Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (ProjectRegistry 19 líneas, ProjectAdoption 22 líneas, boot `uid`, G1–G5 aislamiento; 18/18 + 360/360 · 3483 · 86s, ALLOW con 2 WARNINGS de seguimiento); frontera rollback `51a7611` | Lectura obligatoria — **A0.5 es la única puerta habilitada (pendiente aprobación formal)** |
...
- **A0.5 bitácora evaluable** → [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) — identidad canónica cross-device, registro/promoción/adopción SA-only, 18 tests (15+3-device) y 2 WARNINGS de seguimiento
...
2. Estado: F0 al 100%; F1.1–F1.5 cerradas; **F1.6 con arquitectura aprobada con modificación DEP-SA-004, A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, 76 acotados, 354/354 · 3409, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`) y **A0.5 ✅ Implementado y técnicamente validado, pendiente de revisión formal de Dirección (2026-08-27)** — `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) — `activeProjectId` local, `projectId` canónico account-level; no hay comportamiento funcional de nómina aceptado (A1–A6 bloqueadas hasta aprobación formal).
3. Próxima acción: **Revisión formal de Dirección de A0.5** (bitácora [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) + diff `51a7611`; verificar §2.2–§2.7 y 2 WARNINGS). Mantener **A1–A6 y F1.7 bloqueadas hasta aprobación formal de A0.5**; no implementar todavía configuración cloud ni comportamiento económico F1.6-B. **A0 ya está cerrado — no requiere nueva revisión; A0.5 ya está implementado — no requiere nueva implementación, solo revisión formal.**
