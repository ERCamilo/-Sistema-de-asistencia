...
| Estado | **F0 100%; F1.1–F1.5 cerradas** · arquitectura F1.6 aprobada con modificación DEP-SA-004 · **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, 12/12, 76 acotados, 354/354 · 3409 PASS, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal de Dirección** — base `51a7611` (18/18, 360/360 · 3483, ALLOW con 2 WARNINGS) + micro-cierre `50343ee` (20/20, 360/360 · 3485 · 0 fallos, ALLOW 0 findings; MC1 purga prefijo + MC2 dead canonicalization) — `activeProjectId` local, `projectId` canónico account-level · **A1–A6 y F1.7 bloqueadas hasta cierre formal de A0.5** |
| Punto de parada | F1.5: **354/354 suites · 3401 tests · 0 fallos** (cerrada y aprobada). Post-A0: **354/354 suites · 3409 tests · 0 fallos · 189.372 s · sin reintentos** — **A0 ✅ Cerrado y aprobado por Dirección 2026-08-26** (RecordKey 13/13, F1.4+A0 12/12, 76 acotados, fresh ALLOW); Post-A0.5: **360/360 suites · 3485 tests · 0 fallos · 86s** — **A0.5 ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27) — base `51a7611` (18/18) + 20/20 + ALLOW 0 findings (MC1+MC2 resueltos)**; **A1–A6 y F1.7 bloqueadas hasta cierre formal de A0.5** |
...
| **F1.6 Nómina multiproyecto** | [`F1.6-nomina-multiproyecto.md`](F1.6-nomina-multiproyecto.md) | Arquitectura aprobada con modificación DEP-SA-004; **A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; 354/354 · 3409, RecordKey 13/13, 76 acotados, fresh ALLOW) · **A0.5 / DEP-SA-004 ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1+MC2 resueltos) — `activeProjectId` local, `projectId` canónico; **A1–A6 bloqueadas hasta cierre formal de A0.5** | F1.7 bloqueada hasta cierre F1.6 (A0.5 → A1–A6) |
...
| **F1.6-A0.5 Bitácora identidad canónica** | [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) | ✅ **Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal** — `51a7611` + `50343ee` (ProjectRegistry 19 líneas, ProjectAdoption 22 líneas + filtro `dead`, `LocalWipeService` purga prefijo, `MainSyncStore` `bumpGeneration`; 20/20 + 360/360 · 3485 · 86s, ALLOW 0 findings; MC1+MC2 resueltos); frontera rollback `51a7611` + `50343ee` | Lectura obligatoria — **A0.5 es la única puerta habilitada (pendiente cierre formal)** |
...
2. Estado: F0 al 100%; F1.1–F1.5 cerradas; **F1.6 con arquitectura aprobada con modificación DEP-SA-004, A0 ✅ Cerrado y aprobado por Dirección el 2026-08-26** (`c7a9e0c` + `333a516`; RecordKey 13/13, 76 acotados, 354/354 · 3409, fresh ALLOW; contrato `explicit > default autoritativo > legacy-unresolved:*`) y **A0.5 ✅ Implementado y técnicamente validado + micro-cierre `50343ee` (2026-08-27), pendiente de cierre formal de Dirección** — `51a7611` + `50343ee` (20/20, 360/360 · 3485, ALLOW 0 findings; MC1+MC2 resueltos §3.1) — `activeProjectId` local, `projectId` canónico account-level; no hay comportamiento funcional de nómina aceptado (A1–A6 bloqueadas hasta cierre formal).
3. Próxima acción: **Revisión formal de Dirección de A0.5 + micro-cierre `50343ee`** (bitácora [`F1.6-A0.5-bitacora.md`](F1.6-A0.5-bitacora.md) + diffs `51a7611` + `50343ee`; verificar §2.2–§2.7 y §3.1, WARNINGS ✅ resueltos — ALLOW 0 findings). Mantener **A1–A6 y F1.7 bloqueadas hasta cierre formal de A0.5**; no implementar todavía configuración cloud ni comportamiento económico F1.6-B. **A0 ya está cerrado — no requiere nueva revisión; A0.5 ya está implementado + micro-cierre técnico completo — solo requiere cierre formal.**
